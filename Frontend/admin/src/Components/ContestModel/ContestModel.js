...
import { ApproveImageData } from "../Columns/Columns";
...
function ApproveImage() {
  ...
  const columns = useMemo(() => ApproveImageData, []);
  const data = useMemo(() => {
    const filteredData = Data.filter(
      (row) =>
        !row.isDeleted && row.paymentStatus==="success" && row.images.some((data) => data.imagestatus === "Pending")
    );
