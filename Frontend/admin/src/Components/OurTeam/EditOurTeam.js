...
function EditOurTeam() {
  ...
  const [selectmember, setSelectmember] = useState(location.state.selectmember);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .put(`${process.env.REACT_APP_API_URL}/api/ourTeam/admin/edit/${id}`, formData)
        .then((res) => {
          console.log(res)
          setName(res.data.name)
          setDesignation(res.data.designation)
          setDescription(res.data.description)
          navigate("/admin/home/OurTeam")
        })
    } catch (error) {
      console.error(error);
    }
      ...
      <div className="bg-[white]  rounded-[0.4rem] shadow-md flex flex-col gap-[20px] px-[20px] py-[20px] overflow-x-scroll scroll">
        ...
        <div>
          <select
            ...
            value={selectmember}
            className="border-[1px] shadow-md w-full h-[45px] px-[10px] outline-none text-black/40  text-[15px]"
          >
            <option value="">Select Member</option>
            <option value="Team Leaders">Team Leaders</option>
            <option value="Team Mentors">Team Mentors</option>
            <option value="Go Glowrius 2021 Panel Member">
              Go Glowrius 2021 Panel Member
            </option>
          </select>
        </div>
        <div>
