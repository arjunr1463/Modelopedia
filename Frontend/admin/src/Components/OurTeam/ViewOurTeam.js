...
import { ViewOur } from "../Columns/Columns";
...
function ViewOurTeam() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/ourTeam/getAll`
      );
  ...
  const handleClickActive = (id) => {
    axios
      .post(`${process.env.REACT_APP_API_URL}/api/ourTeam/admin/active/${id}`)
      .then(() => fetchData());
  };
  const handleClickInactive = (id) => {
    axios
      .post(`${process.env.REACT_APP_API_URL}/api/ourTeam/admin/Inactive/${id}`)
      .then(() => fetchData());
  };
  const handleClickDelete = (key) => {
    axios
      .delete(`${process.env.REACT_APP_API_URL}/api/ourTeam/admin/admindelete/${key}`)
      .then(() => fetchData());
  };
                  ...
                  <td className="border-[1px]">
                    <div className="flex flex-col gap-[10px] relative">
                      ...
                      {selectedRow === id && action && (
                        <div className="flex justify-center absolute top-[35px] right-[100px]">
                          <div className="bg-white shadow-md px-[10px] py-[10px] rounded-[0.2rem] w-[150px] gap-[5px] text-[14px] flex flex-col items-start">
                            ...
                            <div className="flex gap-[10px] items-center">
                              ...
                              <Link
                                to={`/admin/home/EditOurTeam/${row.original._id}`}
                                state={{
                                  name: `${row.original.name}`,
                                  description: `${row.original.description}`,
                                  designation: `${row.original.designation}`,
                                  selectmember: `${row.original.selectmember}`,
                                }}
                              >
                                Edit
                              </Link>
